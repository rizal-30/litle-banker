-- Setoran pinjaman tunai: cek hutang nasabah (selaras dengan bayar dari tabungan)
...
BEGIN
  ...
  IF NEW.jenis = 'pinjaman_kembali' AND NOT NEW.bayar_dari_tabungan THEN
    IF NEW.nasabah_id IS NULL THEN
      RAISE EXCEPTION 'Nasabah wajib untuk setoran pinjaman.';
    END IF;

    SELECT COALESCE(hutang, 0), nama
    INTO v_hutang, v_nama_nasabah
    FROM v_saldo_nasabah
    WHERE id = NEW.nasabah_id;
...
COMMENT ON FUNCTION fn_cek_saldo_non_negatif() IS
  'Tolak saldo saku minus, penarikan melebihi tabungan, setoran pinjaman melebihi hutang, dan aturan saku per jenis.';
