"use client";

import { Badge } from "@/components/ui/badge";
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table";
import { DataTableCard } from "@/components/data-table-card";
import { JenisTransaksiBadge } from "@/components/jenis-transaksi-badge";
import { MutasiNasabahJumlah } from "@/components/mutasi-nasabah-jumlah";
import { keteranganMutasiNasabah } from "@/lib/mutasi-nasabah";
import type { Transaksi } from "@/types/database";
import { formatTanggal } from "@/lib/format";
import { cn } from "@/lib/utils";

function MutasiStatusBadges({ t }: { t: Transaksi }) {
  return (
    <div className="flex flex-wrap items-center gap-1">
      <JenisTransaksiBadge jenis={t.jenis} className="text-[11px]" />
      {t.dibatalkan_pada && (
        <Badge variant="outline" className="text-[10px] font-normal">
          Dibatalkan
        </Badge>
      )}
    </div>
  );
}

function MutasiNasabahMobileCard({ t }: { t: Transaksi }) {
  const keterangan = keteranganMutasiNasabah(t);

  return (
    <div
      className={cn(
        "rounded-lg border px-2.5 py-2",
        t.dibatalkan_pada && "opacity-60"
      )}
    >
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-2 gap-y-1">
        <div className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5">
          <MutasiStatusBadges t={t} />
          <span className="text-muted-foreground text-[11px]">
            {formatTanggal(t.tanggal)}
          </span>
        </div>
        <MutasiNasabahJumlah
          jenis={t.jenis}
          jumlah={Number(t.jumlah)}
          bayarDariTabungan={t.bayar_dari_tabungan}
          dibatalkan={!!t.dibatalkan_pada}
          className="text-right text-sm leading-tight whitespace-nowrap"
        />
        {keterangan !== "—" && (
          <p className="text-muted-foreground col-span-1 min-w-0 truncate text-xs leading-snug">
            {keterangan}
          </p>
        )}
      </div>
    </div>
  );
}

interface MutasiNasabahListProps {
  rows: Transaksi[];
}

export function MutasiNasabahList({ rows }: MutasiNasabahListProps) {
  return (
    <>
      <div className="space-y-1.5 sm:hidden">
        {rows.map((t) => (
          <MutasiNasabahMobileCard key={t.id} t={t} />
        ))}
      </div>

      <DataTableCard className="hidden sm:block">
        <Table className="table-fixed">
          <TableHeader>
            <TableRow>
              <TableHead className="w-28 pl-4">Tanggal</TableHead>
              <TableHead className="w-36">Jenis</TableHead>
              <TableHead>Keterangan</TableHead>
              <TableHead className="w-36 pr-4 text-right">Jumlah</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.map((t) => (
              <TableRow
                key={t.id}
                className={cn(t.dibatalkan_pada && "opacity-60")}
              >
                <TableCell className="text-muted-foreground pl-4 text-sm">
                  {formatTanggal(t.tanggal)}
                </TableCell>
                <TableCell>
                  <MutasiStatusBadges t={t} />
                </TableCell>
                <TableCell className="text-muted-foreground max-w-0 truncate">
                  {keteranganMutasiNasabah(t)}
                </TableCell>
                <TableCell className="pr-4 text-right">
                  <MutasiNasabahJumlah
                    jenis={t.jenis}
                    jumlah={Number(t.jumlah)}
                    bayarDariTabungan={t.bayar_dari_tabungan}
                    dibatalkan={!!t.dibatalkan_pada}
                  />
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      </DataTableCard>
    </>
  );
}
